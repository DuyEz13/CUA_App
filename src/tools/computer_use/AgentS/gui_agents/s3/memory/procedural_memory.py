...
class PROCEDURAL_MEMORY:
    ...
    def construct_simple_worker_procedural_memory(agent_class, skipped_actions):
        ...
        **Usage Strategy**:
        ...
        - For tasks requiring simple calculations, DO NOT call code agent or using GUI agent to open Calculation application. Perform calculating BY YOURSELF.
        - When reading and/or extracting data from a data source (e.g., an image, PDF file, or Word document), You MUST follow these instructions. DO NOT zoom in by Ctr + + or any zoom button (+), zoom setting of that browsers. You MUST move the mouse cursor exactly near the data need to be extracted and perform scroll up 2000 3 times to zoom in. You MUST scroll up three times, do not conclude the action didn't work after the first scrolling. Because the first scrolling will make the image smaller but the second and third time will zoom it. After accurately identifying the required data, you MUST scroll down 2 TIMES to zoom back out to the original view. This zoom in and zoom out procedure MUST be executed SEPARATELY and INDEPENDENTLY for EACH individual data point or attribute to be extracted. You MUST NOT assume that a previous zoom action for one attribute is sufficient for another attribute. If zooming in to extract attribute A causes other attributes (e.g., attribute B) to be obscured, partially hidden, or unreadable, you MUST repeat the full zoom-in procedure again specifically for attribute B before extracting its value. Under NO circumstances should you extract any data point without performing the complete zoom-in sequence on that specific data point.
    ...
    IMPORTANT: The system includes a special zooming procedure for image from Google Drive link. If you see after a scroll, the image is smaller, that is LEGITIMATE results of the zooming procedure, not errors or corruption, so DO NOT encourage the computer agent to modify their action.
